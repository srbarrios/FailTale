...
class MinionCollector(BaseCollector):
    ...
    def collect(self, data_definitions):
        """
        Collects data specifically for a minion host using SSH.

        Args:
            data_definitions (list): List of data definitions [{'description': ..., 'command': ...}]
                                     specific to the 'minion' role from config.yaml.
        ...
        """
        results = []
        role = "minion" # Define role for logging clarity
    ...
    # --- Optional minion-specific helper methods ---
    # def _parse_specific_minion_log(self, log_output):
