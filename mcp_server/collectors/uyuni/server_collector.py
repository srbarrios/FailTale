...
class ServerCollector(BaseCollector):
    """Collector implementation for hosts with the 'server' role."""

    def collect(self, data_definitions):
        """
        Collects data specifically for a server host using SSH.

        Args:
            data_definitions (list): List of data definitions [{'description': ..., 'command': ...}]
                                     specific to the 'server' role from config.yaml.
        ...
        """
        results = []
        role = "server" # Define role for logging clarity
    ...
    # --- Optional server-specific helper methods ---
    # def _parse_specific_server_log(self, log_output):
