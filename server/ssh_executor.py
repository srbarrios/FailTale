...
logging.basicConfig(level=logging.WARN, format="%(asctime)s - %(levelname)s - %(message)s", stream=sys.stdout, force=True)
...
async def execute_remote_command_async(
    ...
    while attempt <= retries:
        ...
        try:
            ...
            log.error(f"[{hostname}] Authentication failed: {e}. No retry.")
            if conn and not conn.is_closed:
                conn.close()
            return None, f"Authentication error: {e}", -1

        except HostKeyNotVerifiable as e:
            log.error(f"[{hostname}] Host key verification failed: {e}. No retry.")
            if conn and not conn.is_closed:
                conn.close()
            return None, f"Host key verification failed: {e}", -6

        except (ConnectionLost, DisconnectError, OSError) as e:
            log.warning(f"[{hostname}] Connection error (attempt {attempt+1}/{retries+1}): {e}")
            last_error = e
            if attempt == retries:
                log.error(f"[{hostname}] Max retries reached due to connection errors.")
                if conn and not conn.is_closed:
                    conn.close()
                return None, f"Connection error after retries: {e}", -5

        except asyncio.TimeoutError:
            log.warning(f"[{hostname}] Command timeout (attempt {attempt+1}/{retries+1}, limit: {cmd_timeout}s)")
            last_error = asyncio.TimeoutError(f"Command timed out after {cmd_timeout}s")
            if attempt == retries:
                log.error(f"[{hostname}] Max retries reached due to command timeouts.")
                if conn and not conn.is_closed:
                    conn.close()
                return None, f"Command timeout after retries (limit: {cmd_timeout}s)", -2

        except asyncssh.Error as e:
            log.warning(f"[{hostname}] AsyncSSH error (attempt {attempt+1}/{retries+1}): {e}")
            last_error = e
            if attempt == retries:
                log.error(f"[{hostname}] Max retries reached due to AsyncSSH errors.")
                if conn and not conn.is_closed:
                    conn.close()
                return None, f"SSH error after retries: {e}", -3

        except Exception as e:
            log.exception(f"[{hostname}] Unexpected error (attempt {attempt+1}/{retries+1}): {e}")
            last_error = e
            if attempt == retries:
                log.error(f"[{hostname}] Max retries reached due to unexpected errors.")
                if conn and not conn.is_closed:
                    conn.close()
        ...
        attempt += 1
        if attempt <= retries:
            log.warning(f"[{hostname}] Retrying in {retry_delay}s...")
            await asyncio.sleep(retry_delay)

    log.error(f"[{hostname}] All retries exhausted. Last error: {last_error}")
